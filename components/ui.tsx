...
export function ActionButton({ children, variant = "primary" }: { children?: React.ReactNode; variant?: "primary" | "secondary" | "danger" }) {
  ...
  return <button className={`rounded-xl px-4 py-2 text-sm font-semibold transition ${klass}`}>{children}</button>;
}
    ...
    <div className="min-h-screen bg-gray-bg command-grid">
      <aside className="fixed inset-y-0 left-0 hidden w-72 bg-navy-dark text-white lg:block">
        ...
        <nav className="space-y-1 p-4">
          {config.nav.map((item) => (
            <a key={item} className={`block rounded-xl px-4 py-3 text-sm font-medium ${item === "Dashboard" ? "bg-white/10 text-white" : "text-slate-300 hover:bg-white/5 hover:text-white"}`} href="#">
              {item}
            </a>
          ))}
        </nav>
        ...
        <header className="sticky top-0 z-20 border-b border-border bg-white/90 px-6 py-4 backdrop-blur">
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex gap-2">
              ...
              <ActionButton>New Work Request</ActionButton>
            </div>
          </div>
        </header>
